...
export default class ProductList extends Component {
    ...
    increment(index){
        ...
        newCount[index] = this.state.count[index] ? this.state.count[index] + 1 : 1;
        this.setState({count: newCount});
    }
    decrement(index){
        let newCount = this.state.count;
        newCount[index] = this.state.count[index] ? this.state.count[index] - 1 : 0;
            this.setState({count: newCount});
    }
                     ...
                     <Tabs>
                        ...
                        {val.table_menu_list.map((data,i) => (
                            ...
                            {data.category_dishes.map((items,i) => (
                                ...
                                <div className="add_remove"><button onClick ={()=>this.increment(i)} type="button" className="btn_cmnn add_btn">+</button> {this.state.count[i] || 0 }<button onClick ={()=>this.decrement(i)} type="button" className="btn_cmnn remov_btn">-</button></div>
